...
def rand_word(categoria):
    if categoria == 1: 
        with open("animais.txt", "rt") as f:
            bank = f.readlines()
        return bank[random.randint(0, len(bank))].strip()
    elif categoria == 2: 
        with open("objetos.txt", "rt") as f:
            bank = f.readlines()
        return bank[random.randint(0, len(bank))].strip()
    else: 
...
def main():
    ...
    categoria = int(input('1 - Animais\n2 - Objetos\n3 - Frutas\nCategoria: '))
    game = Hangman(rand_word(categoria))
    print(board[0])
    print('Palavra secreta: ')
    game.hide_word()
    print('\n')
    ...
    print(f'A palavra era: {game.word.capitalize()}')
